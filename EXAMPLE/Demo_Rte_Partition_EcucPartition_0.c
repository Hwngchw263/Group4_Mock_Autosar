...
//chuyển đổi chế độ của task, có thể 
FUNC(void, RTE_CODE_EcucPartition_0) Rte_PostModeTransition_EcucPartition_0(CONST(EventMaskType, AUTOMATIC) event ) {

    //local variable now_task save ID of current task,
    //initial value = 0
    VAR(TaskType, AUTOMATIC) now_task;
    //local variable save current event of now_task 
    //initial value = 0
    VAR(EventMaskType, AUTOMATIC) now_ev;

    now_task = 0;
    now_ev = 0U;
    //gettaskid take ID of current task , store in now_task
    if( E_OK == GetTaskID( &now_task ) ){
        //function to get current event of now_task and store in now_ev
        (VAR(void, AUTOMATIC))GetEvent( now_task, &now_ev );
        //CHECK any event in this line and have at least 1 event 
...
/* Note        :                                                              */
/******************************************************************************/
//bắt đầu 1 section nơi code được đặt
#define RTE_START_SEC_CODE_EcucPartition_0
#include "Rte_MemMap.h"

//define a function
//void is returntype
//function : Rte_Executable_MonitoringTimeout_EcucPartition_0
//manage monitoring, timeout
//VAR(void, AUTOMATIC)
...
//a task with frequent = 20ms
TASK (ASWTask_20ms) {
    //event to save waitting event
    VAR(EventMaskType, AUTOMATIC) Event;

    while( 1 )
    {   
        //function to wait specifi event OsEvent_ASW_20ms
        //it'll block until it happent
        (VAR(void, AUTOMATIC))WaitEvent( OsEvent_ASW_20ms );
        Event = 0U;
        //when event occur , save this event to event
        ...
        if( (Rte_status == RTE_STATUS_RUN) && (Rte_partition_status_EcucPartition_0 == RTE_PARTITION_STATUS_RUNNING) && (SchM_status == SCHM_STATUS_RUN) ) {
            if( (Event & OsEvent_ASW_20ms) > 0U ) {
                //clear event OsEvent_ASW_20ms
                (VAR(void, AUTOMATIC))ClearEvent( OsEvent_ASW_20ms );
            
            //check if event occur regularly
                if ( Rte_periodically_event_activation == TRUE ) {
                    //check if event is disable
                    if( Rte_DisableEventList[3] == 0U ) {
                        //if condition is true, call this function
                        Rte_App_ComTxRx();
                    }
                }
                if ( Rte_periodically_event_activation == TRUE ) {
    ...
    {   //wait for event OS
        ...
        if( (Rte_status == RTE_STATUS_RUN) && (Rte_partition_status_EcucPartition_0 == RTE_PARTITION_STATUS_RUNNING) && (SchM_status == SCHM_STATUS_RUN) ) {
            if( (Event & OsEvent_ComBusDiagTask_100ms) > 0U ) {
                //if event happen, clear event
                (VAR(void, AUTOMATIC))ClearEvent( OsEvent_ComBusDiagTask_100ms );
                //check if event is activate regulaly
        ...
        //if can not afford the first condition
